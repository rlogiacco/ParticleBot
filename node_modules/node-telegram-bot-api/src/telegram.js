 ...
 * @return {Promise}
 */
TelegramBot.prototype.waitResponse = function(message, timeout) {
  var id = message.chat.id + ',' + message.from.id;
  ...
  return function() {
    return new Promise(function (resolve, reject) {
      ...
      debug('Waiting response for %s', id);
    })
    .cancellable()
    .timeout(timeout)
    .catch(Promise.TimeoutError, function(err) {
      debug('Response for %s timed out', id);
      if (id !== '') {
        waiting[id].reject();
        delete waiting[id];
      }
...
TelegramBot.prototype.onCommand = function (command, callback) {
  this.textRegexpCallbacks.push({regexp: new RegExp('/' + command + '(?: (.*))?'), callback: callback, argsre: /(?:(?:\"([^\"]+)\"\s?)|(?:\'([^\']+)\'\s?)|(?:([^\s]+)\s?))/g});
};
